#include <lvgl_pie_chart.h>

namespace ucr { namespace bcoe { namespace cs { namespace cs122 {

    void anim_move_cb(void * var, int32_t v) {
    ...
    void anim_cleanup_cb(lv_anim_t * a) {
        lv_free(a->var);
    }

    void arc_click_cb(lv_event_t * e) {
        lv_obj_t * arc  = lv_event_get_target_obj(e);
        slice_info_t * info = (slice_info_t *)lv_event_get_user_data(e);

        auto active_info = info->parent->active_info;
        auto active_arc = info->parent->active_arc;
    ...
    void LVGL_PieChart::create_slice(lv_obj_t * parent, int percentage, lv_color_t color)
    {
    ...
    void LVGL_PieChart::lv_example_arc_3(void)
    {
