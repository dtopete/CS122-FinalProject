#include <lvgl_compass.h>
#include "lv_conf.h"
#include <lvgl.h>

namespace ucr { namespace bcoe { namespace cs { namespace cs122 {

    const char * LVGL_Compass::heading_to_cardinal(int32_t heading) {
    ...
    void set_heading_value(void * obj, int32_t v) {
        lv_anim_t *anim = reinterpret_cast<lv_anim_t *>(obj);
        LVGL_Compass *comp = reinterpret_cast<LVGL_Compass *>(lv_anim_get_user_data(anim));
        lv_scale_set_rotation(comp->scale, 270 - v);
        lv_label_set_text_fmt(comp->label, "%d°\n%s", v, comp->heading_to_cardinal(v));
    }
     ...
     */
    void LVGL_Compass::lv_example_scale_12(void) {
        ...
        lv_anim_set_user_data(&anim_scale, this);
        lv_anim_start(&anim_scale);

    }

    uint32_t LVGL_Compass::run() {
        lv_example_scale_12();
        return loop();
    }
